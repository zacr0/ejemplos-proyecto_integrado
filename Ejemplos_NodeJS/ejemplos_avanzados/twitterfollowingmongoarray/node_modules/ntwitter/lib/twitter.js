...
Twitter.prototype.get = function(url, params, callback) {
  ...
  console.log('URL GET ' + url + '?' + querystring.stringify(params) );
...
Twitter.prototype.getFriendsIds = function(id, callback) {
  ...
  var params = { key: 'ids' };
  if (/^[0-9]+$/i.test(id)) // If es un número
    params.user_id = id; // El ID se lo asignas a user_id
  else{
    params.screen_name = id; // Se lo asignas a su nombre
  }
...
Twitter.prototype._getUsingCursor = function(url, params, callback) {
  ...
  function fetch(err, data) {
    if (err) {
      params.nextCursor = getNextCursor;
    ...
    if (data.next_cursor_str === '0') {
      ...
      getNextCursor = data.next_cursor_str;
